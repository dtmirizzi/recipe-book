...
export function ReviewForm({ jobId, draft }: { jobId: string; draft: RecipeDraft }) {
  ...
  async function onSave(e: React.FormEvent) {
    ...
    try {
      const res = await fetch('/api/recipes', {
        ...
        body: JSON.stringify({ jobId, draft: d }),
      });
      if (!res.ok) {
    ...
    <form onSubmit={onSave} className="flex flex-col gap-6">
          ...
          {d.ingredients.map((ing, i) => (
            <li key={i} className="flex gap-2 items-center">
              <input
                ...
                style={
                  ing.confidence !== undefined && ing.confidence < 0.6
                    ? { borderColor: 'var(--saffron-500)', background: 'oklch(0.96 0.03 82)' }
                    : undefined
                }
      ...
      <div className="card flex flex-col gap-3">
        ...
        <button
          type="button"
          className="btn"
          onClick={() => update('steps', [...d.steps, { body: '' }])}
        >
          + Add step
        </button>
      </div>
