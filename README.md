...
| Var | What it enables |
|---|---|
| `OPENROUTER_API_KEY` | Real LLM calls via OpenRouter for recipe extraction and smart-search query parsing. Defaults to `google/gemini-2.5-flash` for both text and vision — cheap, multimodal, ~15× less than Claude Sonnet 4.5. Override via `OPENROUTER_MODEL_TEXT` / `OPENROUTER_MODEL_VISION` to use any OpenRouter model id. The API key is read only by `lib/ai/openrouter.ts` (server-only); `lib/env.ts` is also marked `server-only` so the value can never end up in the client bundle. |
...
- **OpenRouter** (any provider — defaults to `google/gemini-2.5-flash` for both text and vision) via the OpenAI SDK with a custom baseURL; deterministic mock extractor when `OPENROUTER_API_KEY` is unset
